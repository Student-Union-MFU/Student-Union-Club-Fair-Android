...
./gradlew assembleRelease   # minified release APK — what a GitHub release ships
...
launches; `make help` lists the rest. Two targets there are not just shorthand:

| | |
|---|---|
| `make bundle` | the AAB Play takes. `make release` stays the APK, because that is what a student downloads from a GitHub release and an AAB is not installable. |
| `make screenshots` | boots the emulator, pins the display to 1080x1920 and renders the store screenshots into `art/play/screenshots/`. See [Store assets](#store-assets). |
...
Everything the Play listing needs lives outside `app/` and ships nowhere:

| | |
|---|---|
| [`PLAY_LISTING.md`](PLAY_LISTING.md) | the listing copy — name, short and full descriptions in `en-US` and `th-TH`, and the App access instructions the review team reads. Character counts are checked against Play's limits in the file. |
| `art/play/icon-512.png` | the store icon. A render of the launcher icon, not a second drawing of it. |
| `art/play/feature-graphic-{en,th}.png` | 1024x500, one per locale. |
| `art/play/screenshots/{en,th}/` | nine per language, regenerated by `make screenshots`. |

The screenshots come out of `StoreScreenshots` in `androidTest` rather than being
captured by hand: the real composables, rendered by the real Compose runtime, so
a store image cannot drift from the app the way a mockup does. They are JPEG
because the backdrop's anti-banding grain defeats PNG — 47 MB a run against 5.6,
for a difference measured at 43.5 dB and visible to nobody. The student in them
is a fixture; `PreviewStudent`'s name, email and id read as a real person's, and
these frames are public.

**Two certificates reach students, and both need registering.** A GitHub release
carries the upload key; a Play install carries Google's app-signing key, because
Play strips the upload signature and re-signs. Google Sign-In binds to package
name plus certificate, so each fingerprint needs its own Android OAuth client for
`com.su.clubfair` — register only one and sign-in breaks for half your users, on
a path you cannot reproduce locally. It also means **an install from Play cannot
upgrade a sideloaded install**: different certificate, so Android refuses with a
bare "App not installed" and the sideloaded copy has to be uninstalled first.
Nothing is lost — checkpoints live on su-server.
...
app/src/androidTest/         # Compose UI / instrumented tests, and StoreScreenshots
```
...
| `su.png` | `drawable-nodpi/logo_su.png`, and the SU monogram traced out of it for `drawable/ic_launcher_foreground.xml` |
